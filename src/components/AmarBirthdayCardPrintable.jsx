...
export default function AmarBirthdayCardPrintable() {
  ...
  const renderCompactWeekGrid = () => {
    ...
    const rowsToShow = 75; // Show all 75 years for complete visualization
    ...
    for (let row = 0; row < rowsToShow; row++) {
      ...
      for (let col = 0; col < weeksPerRow; col++) {
        ...
        // Enhanced styling similar to interactive tab
        let cellClass = "w-1.5 h-1.5 m-0.5 rounded-full transition-all duration-300 ";
        if (isPast) {
          if (row < 5) { // Childhood (0-5 years)
            cellClass += showAnimation ? "bg-yellow-400 shadow-sm" : "bg-gray-200";
          } else if (row < 18) { // Youth (5-18 years)  
            cellClass += showAnimation ? "bg-green-500 shadow-sm" : "bg-gray-200";
          } else if (row < 29) { // Education (18-29 years)
            cellClass += showAnimation ? "bg-blue-500 shadow-sm" : "bg-gray-200";
          } else { // Professional life (29+ years)
            cellClass += showAnimation ? "bg-amber-600 shadow-sm" : "bg-gray-200";
          }
        } else if (isCurrent) {
          ...
          <div className="relative z-10">
            <div className="mb-8">
              ...
              <div className="w-48 h-48 mx-auto rounded-full bg-white/20 backdrop-blur-sm p-2 mb-6">
                <img 
                  src="/portrait.png" 
                  alt="Amar Singh Dhiman - Portrait" 
                  className="w-full h-full rounded-full object-cover border-4 border-white/50 shadow-2xl"
                  onError={(e) => {
                    e.target.style.display = 'none';
                    e.target.nextSibling.style.display = 'flex';
                  }}
                />
                <div className="w-full h-full rounded-full bg-gradient-to-br from-amber-400 to-green-600 flex items-center justify-center hidden">
          ...
          <div className="border-2 border-green-200 rounded-2xl p-8 bg-gradient-to-br from-green-50 to-amber-50">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm mt-6">
              <div className="flex items-center justify-center">
                ...
                <span>Childhood (0-5)</span>
              </div>
              <div className="flex items-center justify-center">
                <div className="w-3 h-3 bg-green-500 rounded-full mr-2"></div>
                <span>Youth (5-18)</span>
              </div>
              <div className="flex items-center justify-center">
                <div className="w-3 h-3 bg-blue-500 rounded-full mr-2"></div>
                <span>Education (18-29)</span>
              </div>
              <div className="flex items-center justify-center">
            ...
            <div className="mt-8 flex justify-center">
              <div className="text-center">
                <img 
                  ...
                  alt="Amar Singh Dhiman - Limited Edition" 
                  className="w-auto h-auto max-w-full max-h-96 object-contain mx-auto shadow-2xl rounded-lg"
                  onError={(e) => {
                    e.target.style.display = 'none';
                  }}
                />
                <p className="text-sm text-gray-600 mt-3 font-medium">Limited Edition</p>
              </div>
            </div>
          </div>
